...
- `Escape`로 종료하면 패널 위치, 분할 비율, 선택과 스크롤 위치 복원
...
상단의 문서 버튼을 누르면 최근에 연 Markdown 문서를 최대 10개까지 확인하고 바로 전환할 수 있습니다. 목록은 기기에 저장되며, 현재 문서에서 수정 중인 Markdown이 있으면 다른 문서로 이동하기 전에 확인합니다. 삭제되었거나 접근할 수 없는 문서는 연결 끊김 상태로 남아 복구 후 다시 열 수 있습니다.

열어 둔 파일이 다른 앱에서 변경되거나 삭제되면 현재 화면을 자동으로 덮어쓰지 않고 알려줍니다. `다시 불러오기`를 선택하면 최신 원본을 검증해 적용하며, Aster 안에서 수정한 Markdown이 있다면 먼저 확인합니다.
